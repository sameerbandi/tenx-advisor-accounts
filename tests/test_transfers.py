...
def test_transfer_negative_amount(client):
    assert _transfer(client, -100).status_code == 422
...
def test_transfer_fractional_cents(client):
    assert _transfer(client, 10.005).status_code == 422


def test_transfer_same_account(client):
    assert _transfer(client, 100, dst="ACC-1001").status_code == 400